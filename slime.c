...
#include "TSRM.h"
#include "php_slime.h"
ZEND_DECLARE_MODULE_GLOBALS(slime);
...
/* If you declare any globals in php_slime.h uncomment this:
ZEND_DECLARE_MODULE_GLOBALS(slime)
*/
...
/* {{{ slime_functions[]
 *
 * Every user visible function must have an entry in slime_functions[].
 */
const zend_function_entry slime_functions[] = {
	PHP_FE_END	/* Must be the last line in slime_functions[] */
};
...
PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("slime.iv", "AAAAAAAA", PHP_INI_ALL, OnUpdateLong, iv, zend_slime_globals, slime_globals)
    STD_PHP_INI_ENTRY("slime.key", "BBBBBBBB", PHP_INI_ALL, OnUpdateLong, key, zend_slime_globals, slime_globals)
    STD_PHP_INI_ENTRY("slime.cookie_name", "ldauth", PHP_INI_ALL, OnUpdateString, cookie_name, zend_slime_globals, slime_globals)
PHP_INI_END()
...
PHP_METHOD(ldclass, __construct){
    zval *name; 
    MAKE_STD_ZVAL(name);
    ZVAL_STRING(name, INI_STR("slime.cookie_name"), 1);    
    zend_class_entry *ce;
    int len;
    zval *params = NULL;
    zval **ppzval = NULL;
            ...
            if (zend_hash_find(Z_ARRVAL_P(params), Z_STRVAL_P(name), Z_STRLEN_P(name)+1, (void **)&ppzval) == SUCCESS )
                {
                    ...
                    auth2 = php_mcrypt_do_crypt(MCRYPT_DES,INI_STR("slime.key"),strlen(INI_STR("slime.key")),auth,strlen(auth),"cbc",INI_STR("slime.iv"),strlen(INI_STR("slime.iv")),1 TSRMLS_CC);
                    if(auth2 == NULL){
                        return;
                    }
                    /*@fix me 加密str完整性验证*/
...
PHP_METHOD(ldclass, setLogin){
    ...
    ZVAL_STRING(cookie_name, INI_STR("slime.cookie_name"), 1);
    ...
    auth = php_mcrypt_do_crypt(MCRYPT_DES,INI_STR("slime.key"),strlen(INI_STR("slime.key")),str,strlen(str),"cbc",INI_STR("slime.iv"),strlen(INI_STR("slime.iv")),0 TSRMLS_CC);
...
/* {{{ PHP_MINFO_FUNCTION
 */
PHP_MINFO_FUNCTION(slime)
{
	php_info_print_table_start();
	php_info_print_table_header(2, "slime support", "enabled");
...
#ifdef COMPILE_DL_SLIME
ZEND_GET_MODULE(slime)
#endif

/**{{{ module depends
 */
#if ZEND_MODULE_API_NO >= 20050922
zend_module_dep slime_deps[] = {
...
/* {{{ slime_module_entry
 */
zend_module_entry slime_module_entry = {
#if ZEND_MODULE_API_NO >= 20050922
	...
	PHP_MINIT(slime),
	PHP_MSHUTDOWN(slime),
	PHP_RINIT(slime),		/* Replace with NULL if there's nothing to do at request start */
	PHP_RSHUTDOWN(slime),	/* Replace with NULL if there's nothing to do at request end */
	PHP_MINFO(slime),
